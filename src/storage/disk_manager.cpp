...
page_id_t DiskManager::AllocatePage() {
  ...
  {
    if(meta_page_information->GetExtentUsedPage(i)<BITMAP_SIZE)   //还有可用的
    {
      //算物理页号（位图页）
      uint32_t bit_map_physical_page = i*(this->BITMAP_SIZE + 1) + 1;//通过位图页的大小，和i（第几个分区）计算物理页
...
bool DiskManager::IsPageFree(page_id_t logical_page_id) {
  ...
  // DiskFileMetaPage* meta_page_information = reinterpret_cast<DiskFileMetaPage*>(this->meta_data_); 
  uint32_t temp_offest_in_exetent = logical_page_id % BITMAP_SIZE;
  uint32_t temp_exetent = logical_page_id / BITMAP_SIZE;//分区
